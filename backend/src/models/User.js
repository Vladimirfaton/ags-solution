...
static async setProfile(userId, { nom, prenom, email, telephone, username: requestedUsername }) {
  ...
  if (current.username_locked) {
    ...
    if (duplicate.rowCount) throw Object.assign(new Error('Cet identifiant est déjà utilisé.'), { status: 409, expose: true });
    username = candidate;
    usernameLocked = true;

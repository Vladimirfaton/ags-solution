...
export class CashRegister {
  ...
  const [yearTotal, byMode, recent, activeYear, enrolled] = await Promise.all([
  ...
  return { today: yearTotal.rows[0], byMode: byMode.rows, recent: recent.rows, activeYear: activeYear.rows[0] || null, enrolledStudents: enrolled.rows[0].count };
}
}

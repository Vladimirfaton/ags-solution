...
export class FinancialStatement {
  static async summary(scope, options = {}) {
    const { sql: filterSql, params } = buildFilters(scope, options, 1);
    const result = await query(`${baseCte(filterSql)}
      SELECT statut, COUNT(*)::int AS total FROM computed GROUP BY statut`, params);
  ...
  static async list(scope, { search = '', classeAnnuelleId = null, statut = 'tous', page = 1, pageSize = 10 } = {}) {
    const { sql: filterSql, params } = buildFilters(scope, { search, classeAnnuelleId }, 1);
    const statutFilter = ['solde', 'partiel', 'impaye'].includes(statut) ? ` WHERE statut = $${params.length + 1}` : '';
    ...
    const [rows, count] = await Promise.all([
      query(`${baseCte(filterSql)}
        ...
        LIMIT $${params.length + statutParams.length + 1} OFFSET $${params.length + statutParams.length + 2}`,
        [...params, ...statutParams, pageSize, offset]),
      query(`${baseCte(filterSql)}
